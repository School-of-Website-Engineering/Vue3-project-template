...
function getgonggao() {
    let mode = "GET";
    let link = domain + "https://ovc.xiaon.life/api/v1/user/notice/fetch";
    let params = "";
    tanjiazai();
...
function getdingyue() {
    let mode = "GET";
    let link = domain + "https://ovc.xiaon.life/api/v1/user/getSubscribe";
    let params = "";
    tanjiazai();
...
function responsedingyue(code, data) {
    if (code == "200") {
        ...
        if (plan == null && plan_id == null) {
            ...
            c.innerHTML =
                "将于" +
                getshijian(expired_at) +
                "日过期,还有" +
                reset_day +
                "天重置流量";
            e.style.width =
                Math.trunc(((u + d) / transfer_enable) * f.offsetWidth) + "px"; //设置进度条的宽度
            dd.innerHTML =
                "已使用" +
                Math.trunc((u + d) / 1024 / 1024 / 1024) +
                "GB/" +
                Math.trunc(transfer_enable / 1024 / 1024 / 1024) +
                "GB";
...
function opengonggao() {
    ...
    // 检查是否设置了暂时关闭通知的时间戳
    var temporaryClosedTime = localStorage.getItem("temporaryClosedTime");
    var currentTime = Date.now();

    // 如果设置了暂时关闭通知的时间戳，并且距离现在不到1小时
    if (temporaryClosedTime && currentTime - temporaryClosedTime < 720000) {
        // 不显示弹窗
        return;
    }
...
function temporarilyclosed() {
    var currentTime = Date.now();
    localStorage.setItem("temporaryClosedTime", currentTime);
...
function xufeianniu() {
    ...
    if (dingyue.plan == null && dingyue.plan_id == null) {
        ...
        ts < dingyue.expired_at &&
        dingyue.expired_at != null &&
        dingyue.u + dingyue.d < dingyue.transfer_enable
    ) {
        window.location.href =
            "./shop_peizhi.html?type=xufei&planid=" + dingyue.plan_id;
        ...
        window.location.href =
            "./shop_peizhi.html?type=chongzhi&planid=" + dingyue.plan_id;
    } else if (
        (dingyue.expired_at == null || dingyue.expired_at == 0) &&
        dingyue.u + dingyue.d < dingyue.transfer_enable
    ) {
        window.location.href =
            "./shop_peizhi.html?type=chongzhi&planid=" + dingyue.plan_id;
...
function gettishi() {
    let mode = "GET";
    let link = domain + "https://ovc.xiaon.life/api/v1/user/getStat";
    let params = "";
    tanjiazai();
