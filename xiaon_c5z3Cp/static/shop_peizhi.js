...
function getinfo() {
    let mode = "GET";
    let link = domain + "https://ovc.xiaon.life/api/v1/user/info";
    let params = "";
    tanjiazai();
...
function getdingyue(planid) {
    let mode = "GET";
    let link =
        domain + "https://ovc.xiaon.life/api/v1/user/plan/fetch?id=" + planid;
    let params = "";
    tanjiazai();
...
function responsedingyue(code, data) {
    if (code == "200") {
        ...
        let price_group = {};
        if (JSON.parse(data).data.month_price != null) {
            price_group["month_price"] = JSON.parse(data).data.month_price;
        }
        if (JSON.parse(data).data.quarter_price != null) {
            price_group["quarter_price"] = JSON.parse(data).data.quarter_price;
        }
        if (JSON.parse(data).data.half_year_price != null) {
            price_group["half_year_price"] = JSON.parse(
                data
            ).data.half_year_price;
        }
        if (JSON.parse(data).data.year_price != null) {
            price_group["year_price"] = JSON.parse(data).data.year_price;
        }
        if (JSON.parse(data).data.two_year_price != null) {
            price_group["two_year_price"] = JSON.parse(
                data
            ).data.two_year_price;
        }
        if (JSON.parse(data).data.three_year_price != null) {
            price_group["three_year_price"] = JSON.parse(
                data
            ).data.three_year_price;
        }
        if (JSON.parse(data).data.onetime_price != null) {
            price_group["onetime_price"] = JSON.parse(data).data.onetime_price;
        }
        ...
        if (qidongcanshu["type"] == "chongzhi") {
            ...
            f.innerHTML =
                "¥" + (JSON.parse(data).data.reset_price / 100).toFixed(2);
            d.appendChild(e);
            d.appendChild(f);
            c.appendChild(d);
        } else {
...
function switchchoose(data) {
    ...
    if (youhuiquan["type"] == "1") {
        ...
        d.innerHTML =
            "¥" + (b * ((100 - youhuiquan["value"]) / 100)).toFixed(2);
    } else {
        d.innerHTML = "¥" + b;
    }
}
...
function checkyouhui() {
    let mode = "POST";
    let link = domain + "https://ovc.xiaon.life/api/v1/user/coupon/check";
...
function tijiaodingdan() {
    let mode = "POST";
    let link = domain + "https://ovc.xiaon.life/api/v1/user/order/save";
