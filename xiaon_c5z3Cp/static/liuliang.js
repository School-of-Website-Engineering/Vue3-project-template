...
function getjilu() {
    let mode = 'GET';
    let link = domain + 'https://ovc.xiaon.life/api/v1/user/stat/getTrafficLog';
    let params = '';
    tanjiazai();
...
//       g.innerHTML = '合计:'+(a[x].u/1024/1024/1024+a[x].d/1024/1024/ 1024*a[x].server_rate).toFixed(2)+'GB';
...
function responsegetjilu(code, data) {
    if (code == '200') {
        ...
        if (a.length > 10) {
            let scrollableBox = document.createElement('div');
            scrollableBox.setAttribute('class', 'scrollable-box');
            ...
            for (x in a) {
                ...
                g.innerHTML =
                    '合计:' +
                    (a[x].u / 1024 / 1024 / 1024 + (a[x].d / 1024 / 1024 / 1024) * a[x].server_rate).toFixed(2) +
                    'GB';
                ...
                scrollableBox.appendChild(b);
            }

            jj.appendChild(scrollableBox);
        } else {
            for (x in a) {
                let b = document.createElement('div');
                b.setAttribute('class', 'order_box');
