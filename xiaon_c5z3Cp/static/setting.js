...
function getconfig() {
    let mode = 'GET';
    let link = domain + 'https://ovc.xiaon.life/api/v1/user/comm/config';
    let params = '';
    tanjiazai();
...
function responseconfig(code, data) {
    if (code == '200') {
        ...
        if (a === null || a === 'null') {
            // 如果 a 为 null 或 "null"，则移除 onclick 属性
            b.removeAttribute('onclick');
            let linkElement = document.querySelector('.set_des_group a');
            if (linkElement) {
            ...
            // 如果 a 不为 null 或 "null"，则设置 onclick 为 a 的值
            b.setAttribute('onclick', 'window.location.href = "' + a + '"');
        }
        canceltanjiazai();
    } else {
        tan('cuowu', '当前网络不稳定，请刷新页面重试');
...
function qiehuankaiguan(zhi) {
    let mode = 'POST';
    let link = domain + 'https://ovc.xiaon.life/api/v1/user/update';
    var params = {};
...
function resetconfig() {
    let mode = 'GET';
    let link = domain + 'https://ovc.xiaon.life/api/v1/user/resetSecurity';
    let params = '';
    tanjiazai();
...
function querenxiugai(zhi) {
    let mode = 'POST';
    let link = domain + 'https://ovc.xiaon.life/api/v1/user/changePassword';
