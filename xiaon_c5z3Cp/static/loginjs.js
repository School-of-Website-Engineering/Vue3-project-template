...
function regrequest() {
    let mode = 'POST';
    let link = domain + 'https://ovc.xiaon.life/api/v1/passport/auth/login';
...
function buildrequest() {
    let mode = 'GET';
    let link = domain + 'https://ovc.xiaon.life/api/v1/guest/comm/config';
    console.log(link);
    params1 = '';
...
function emailrequest() {
    let mode = 'POST';
    let link = domain + 'https://ovc.xiaon.life/api/v1/passport/comm/sendEmailVerify';
...
function yanzhengtime() {
    ...
    if (emailtime > 0) {
        ...
        if (emailtime === 55) {
            ...
            hintElement.textContent = '没收到验证码？去邮件垃圾箱看看';
            hintElement.id = 'hintText';
            hintContainer.appendChild(hintElement);
            a.parentNode.insertBefore(hintContainer, a); // 在邮箱按钮之前插入 hintContainer
        }
        ...
        if (hintContainer && hintContainer.parentNode) {
            hintContainer.parentNode.removeChild(hintContainer); // 删除 hintContainer
        }
    }
}
...
function zhucerequest() {
    let mode = 'POST';
    let link = domain + 'https://ovc.xiaon.life/api/v1/passport/auth/register';
    let a = document.getElementById('emailinput');
...
function yanzhengtime_forgot() {
    ...
    if (emailtime > 0) {
        ...
        if (emailtime === 55) {
            ...
            hintText.innerHTML = '没收到验证码？去邮件垃圾箱看看';
            hintContainer.appendChild(hintText);
            a.parentNode.insertBefore(hintContainer, a.nextSibling);
        }
...
function forgotrequest() {
    let mode = 'POST';
    let link = domain + 'https://ovc.xiaon.life/api/v1/passport/auth/forget';
    let a = document.getElementById('forgotemail');
